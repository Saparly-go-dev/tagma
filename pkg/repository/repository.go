...
type Authorization interface {
	...
	GetUser(username string) (tagma.User, error)
	GetUserByID(userID int) (tagma.User, error)
	UpdatePassword(userID int, password string) error
	GetAgentId(Id int) (int, error)
...
func NewRepository(db *gorm.DB) *Repository {
	return &Repository{
		...
		Trade_Channel:      NewTradeChannelPostgres(db),
		Trade_Category:     NewTradeCategoryPostgres(db),
		Test:               NewTestPostgres(db),
		ProductType:        NewProductTypePostgres(db),
		Product:            NewProductPostgres(db),
		Driver:             NewDriverPostgres(db),
		Ekspeditor:         NewEkspeditorPostgres(db),
		TradeAgent:         NewTradeAgentPostgres(db),
		TradePoint:         NewTradePointPostgres(db),
		Contact:            NewContactPostgres(db),
		Route:              NewRoutePostgres(db),
		Log:                NewLogPostgres(db),
		Order:              NewOrderPostgres(db),
		Discount:           NewDiscountPostgres(db),
		Gift:               NewGiftPostgres(db),
		District:           NewDistrictPostgres(db),
		Note:               NewNotePostgres(db),
		User:               NewUserPostgres(db),
		Payment:            NewPaymentPostgres(db),
		Marketing:          NewMarketingPostgres(db),
		Manual:             NewManualPostgres(db),
		Dealer:             NewDealerPostgres(db),
		DealerProduct:      NewDealerProductPostgres(db),
		DealerOrder:        NewDealerOrderPostgres(db),
		TimeChecker:        NewTimeCheckerPostgres(db),
		Promotion:          NewPromotionPostgres(db),
		ExtraTradePointDay: NewExtraTradePointDayPostgres(db),
		ExtraOrder:         NewExtraOrderPostgres(db),
		Plan:               NewPlanPostgres(db),
		Furniture:          NewFurniturePostgres(db),
		Merchandiser:       NewMerchandiserPostgres(db),
		TradePointDebt:     NewTradePointDebtPostgres(db),
		Promo:              NewPromoPostgres(db),
	}
}
